/* linux/arch/arm/plat-s5pc1xx/ltc3714.c
 *
...
static int ltc3714_gpio_setting(void)
{
	...
	for (iter = SER_BIT; iter <= VID_CTRL_BIT ; iter++) {
		if (gpio_is_valid(S5PC1XX_GPH0(iter))) {
			...
			if (err) {
				printk(KERN_ERR "failed to get GPF0(%d)\n", iter);
				return err;
			}
...
static int set_ltc3714(unsigned int pwr, enum perf_level p_lv)
{
	...
	if (pwr == PMIC_ARM) {
		...
		printk(KERN_ERR "[error]: set_power, check mode [pwr] value\n");
		return -EINVAL;
	}

	for (iter = 8; iter > 0; iter--) {
		gpio_set_value(S5PC1XX_GPH0(SRCLK_BIT), 0);
		gpio_set_value(S5PC1XX_GPH0(SER_BIT),
					(gpio_val>>(iter-1)) & 0x01);
		udelay(10);
...
static int find_voltage(int freq)
{
	...
	printk(KERN_ERR "Cannot find matched voltage on table\n");
