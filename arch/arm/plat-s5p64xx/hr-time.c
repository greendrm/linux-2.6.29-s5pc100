...
static inline void s5p64xx_tick_set_autoreset(void)
{
	unsigned long tcon;
...
static inline void s5p64xx_tick_remove_autoreset(void)
{
	unsigned long tcon;
			      ...
			      struct clock_event_device *evt)
{
	...
	case CLOCK_EVT_MODE_PERIODIC:
		s5p64xx_tick_set_autoreset();
		break;
	case CLOCK_EVT_MODE_ONESHOT:
		s5p64xx_tick_timer_stop();
		s5p64xx_tick_remove_autoreset();
		break;
...
static void __init  s5p64xx_init_dynamic_tick_timer(unsigned long rate)
{
...
static unsigned long s5p64xx_mpu_timer2_overflows;

irqreturn_t s5p64xx_mpu_timer2_interrupt(int irq, void *dev_id)
{

	unsigned long tcstat;

	s5p64xx_mpu_timer2_overflows++;

	return IRQ_HANDLED;
}

struct irqaction s5p64xx_timer2_irq = {
	.name		= "pwm_timer2",
	.flags		= IRQF_DISABLED ,
	.handler	= s5p64xx_mpu_timer2_interrupt,
};
...
static void __init s5p64xx_init_clocksource(unsigned long rate)
{
 ...
 * ---------------------------------------------------------------------------
 */
static void s5p64xx_dynamic_timer_setup(void)
{
...
static void __init s5p64xx_dynamic_timer_init(void)
{
	s5p64xx_dynamic_timer_setup();
	setup_irq(IRQ_TIMER2, &s5p64xx_timer2_irq);
	setup_irq(IRQ_TIMER4, &s5p64xx_tick_timer_irq);
}


struct sys_timer s5p64xx_timer = {
	.init		= s5p64xx_dynamic_timer_init,
};
