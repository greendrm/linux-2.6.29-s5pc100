...
static unsigned int s5p64xx_systimer_write(unsigned int *reg_offset, unsigned int value)
{
 ...
 */
static unsigned long s5p64xx_gettimeoffset (void)
{
	unsigned long tdone;
	unsigned long tval;
	...
	tval = s5p64xx_systimer_read(S3C_SYSTIMER_ICNTO) * timer_startval;
	tval += s5p64xx_systimer_read(S3C_SYSTIMER_TCNTO);

	tdone = clk_tick_totcnt - tval;

	/* check to see if there is an interrupt pending */
	if (s5p64xx_ostimer_pending()) {
 ...
 */
static irqreturn_t s5p64xx_timer_interrupt(int irq, void *dev_id)
{
...
static void s5p64xx_timer_setup (void)
{
	...
	s5p64xx_systimer_write(S3C_SYSTIMER_ICNTB, timer_icnt);
	...
	s5p64xx_systimer_write(S3C_SYSTIMER_INT_CSTAT, (S3C_SYSTIMER_INT_ICNTEIE|S3C_SYSTIMER_INT_EN));
}

static void __init s5p64xx_timer_init(void)
{
	s5p64xx_timer_setup();
	setup_irq(IRQ_SYSTIMER, &s5p64xx_timer_irq);
}

struct sys_timer s5p64xx_timer = {
	.init		= s5p64xx_timer_init,
	.offset		= s5p64xx_gettimeoffset,
	.resume		= s5p64xx_timer_setup
};
