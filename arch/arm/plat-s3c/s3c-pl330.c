/* linux/arch/arm/plat-s3c/s3c-pl330.c
 *
...
int s3c2410_dma_ctrl(enum dma_ch id, enum s3c_chan_op op)
{
	struct s3c_pl330_xfer *xfer;
...
int s3c2410_dma_config(enum dma_ch id, int xferunit, int dcon)
{
	struct s3c_pl330_chan *ch;
	struct pl330_info *pi;
	unsigned long flags;
...
int s3c2410_dma_devconfig(enum dma_ch id, enum s3c2410_dmasrc source,
			  int hwcfg, unsigned long address)
{
	struct s3c_pl330_chan *ch;
	unsigned long flags;
	int ret = 0;
