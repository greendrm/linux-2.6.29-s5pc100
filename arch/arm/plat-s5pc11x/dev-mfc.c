...
void __init s3c_mfc_set_platdata(struct s3c_mfc_platdata *pd)
{
	...
	if(pd) {
		set->buf_phys_base[0] = pd->buf_phys_base[0];
		set->buf_phys_base[1] = pd->buf_phys_base[1];
		set->buf_phys_size[0] = pd->buf_phys_size[0];
		set->buf_phys_size[1] = pd->buf_phys_size[1];
	}
}
