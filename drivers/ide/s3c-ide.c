...
static u8 s3c_ide_ratefilter(ide_drive_t * drive, u8 speed)
{
	...
	case XFER_UDMA_6:
	case XFER_UDMA_5:
		/* S5PC1OO and S5PC110 can support upto UDMA4 */
		speed = XFER_UDMA_4;
		break;
	case XFER_UDMA_4:
	case XFER_UDMA_3:
...
static void s3c_ide_tune_chipset(ide_drive_t * drive, u8 xferspeed)
{
	...
	if (XFER_PIO_0 <= speed && speed <= XFER_PIO_4) {
		...
		writel(s3c_hwif->piotime[speed - XFER_PIO_0],
		       s3c_ide_regbase + S5P_ATA_PIO_TIME);
	} else {
		writel(s3c_hwif->piotime[0],
		       s3c_ide_regbase + S5P_ATA_PIO_TIME);
...
int s3c_ide_irq_hook(void *data)
{
	...
	if (s3c_hwif->pseudo_dma) {
		...
		if (reg & 0x10) {
			...
			writel(s3c_hwif->table[i].len - 0x1,
			       s3c_ide_regbase + S5P_ATA_TBUF_SIZE);
			writel(s3c_hwif->table[i].addr,
			       s3c_ide_regbase + S5P_ATA_TBUF_START);
		} else {
...
/* GPIO Settings */
static void s3c_ide_setup_gpio(void)
{
	unsigned long reg;
	...
	/*CF_Data[0 - 7] */
	writel(0x44444444, S5PC11X_GPJ2CON);
	writel(0x0, S5PC11X_GPJ2PUD);
	writel(0xffff, S5PC11X_GPJ2DRV);

	/* CF_Data[8 - 15] */
	writel(0x44444444, S5PC11X_GPJ3CON);
	writel(0x0, S5PC11X_GPJ3PUD);
	writel(0xffff, S5PC11X_GPJ3DRV);

	/* CF_CS0, CF_CS1, CF_IORD, CF_IOWR */
	reg = readl(S5PC11X_GPJ4CON) & ~(0xffff);
	writel(reg | 0x4444, S5PC11X_GPJ4CON);
	reg = readl(S5PC11X_GPJ4PUD) & ~(0xff);
	writel(0xff, S5PC11X_GPJ4DRV);
	mdelay(100);
#elif defined(CONFIG_CPU_S5PC100)
	reg = readl(S5P_MEM_SYS_CFG) & ~(0x3f);
	writel(reg | 0x30, S5P_MEM_SYS_CFG);
...
static void __devinit s3c_ide_setup_ports(hw_regs_t * hw,
					  s3c_ide_hwif_t * s3c_hwif)
{
	int i;
	...
	for (i = 0; i < 9; i++)
		...
		*ata_regs++ = (ulong)s3c_ide_regbase + 0x54 + (i << 2);
#endif
}
...
static void s3c_ide_init_hwif(void)
{
#if defined (CONFIG_CPU_S5PC100)
	unsigned long reg;
...
static int __devinit s3c_ide_probe(struct platform_device *pdev)
{
	...
	if (!request_mem_region
	    (s3c_ide_res->start, s3c_ide_res->end - s3c_ide_res->start + 1,
	     pdev->name)) {
	...
	s3c_ide_regbase = ioremap(s3c_ide_res->start,
		    s3c_ide_res->end - s3c_ide_res->start + 1);
	...
	/* Calculates timing parameters for PIO and UDMA modes */
