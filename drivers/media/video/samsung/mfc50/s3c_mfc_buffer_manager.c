...
//extern volatile unsigned char *s3c_mfc_virt_dpb_luma_buf;	// port 0
...
static unsigned int s3c_mfc_get_mem_area(int allocSize, int inst_no, int port_no)
{
	...
	for (node = s3c_mfc_free_mem_head[port_no]; node != s3c_mfc_free_mem_tail[port_no]; node = node->next) {
		if (match_node != NULL) {
