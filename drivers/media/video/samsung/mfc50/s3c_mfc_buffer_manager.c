...
void s3c_mfc_print_list(void)
{
	...
	for (port_no = 0; port_no < MFC_MAX_PORT_NUM; port_no++) {
		...
		for (node1 = s3c_mfc_alloc_mem_head[port_no];
			...
			printk
			    ("s3c_mfc_print_list [AllocList][%d] inst_no : %d port_no : %d  p_addr : 0x%08x v_addr:0x%08x size:%d\n",
		...
		for (node2 = s3c_mfc_free_mem_head[port_no];
		     ...
		     node2 = node2->next) {
			printk
			    ("s3c_mfc_print_list [FreeList][%d] startAddr : 0x%08x size:%d\n",
...
static void s3c_mfc_insert_first_node_to_free_list(s3c_mfc_free_mem_t * node,
						   int inst_no, int port_no)
{
...
static void s3c_mfc_insert_node_to_free_list(s3c_mfc_free_mem_t * node,
					     int inst_no, int port_no)
{
...
static void s3c_mfc_del_node_from_alloc_list(s3c_mfc_alloc_mem_t * node,
					     int inst_no, int port_no)
{
	mfc_debug("[%d]instance, [%d]port (p_addr : 0x%08x size:%d)\n",
		  inst_no, port_no, node->p_addr, node->size);
...
static void s3c_mfc_del_node_from_free_list(s3c_mfc_free_mem_t * node,
					    int inst_no, int port_no)
{
	mfc_debug
	    ("[%d]s3c_mfc_del_node_from_free_list(startAddr : 0x%08x size:%d)\n",
	     inst_no, node->start_addr, node->size);
		...
		while (node1 != s3c_mfc_free_mem_tail[port_no]) {
			...
			while (node2 != s3c_mfc_free_mem_tail[port_no]) {
				if (node1->start_addr + node1->size ==
					...
					s3c_mfc_del_node_from_free_list(node2,
									inst_no,
									port_no);
					break;
...
int s3c_mfc_init_buffer_manager(void)
{
	...
	for (port_no = 0; port_no < MFC_MAX_PORT_NUM; port_no++) {
		...
		alloc_node =
		    (s3c_mfc_alloc_mem_t *) kmalloc(sizeof(s3c_mfc_alloc_mem_t),
						    GFP_KERNEL);
		...
		free_node =
		    (s3c_mfc_free_mem_t *) kmalloc(sizeof(s3c_mfc_free_mem_t),
						   GFP_KERNEL);
		...
		free_node->size =
		    (port_no) ? s3c_mfc_dpb_luma_buf_size : s3c_mfc_buf_size;
					    ...
					    s3c_mfc_args * args)
{
	int ret;
	s3c_mfc_alloc_mem_t *node;
	s3c_mfc_get_phys_addr_arg_t *codec_get_phy_addr_arg =
	    (s3c_mfc_get_phys_addr_arg_t *) args;
