...
void ipc_set_polyphase_filter(volatile u32 filter_reg, 
					const s8* filter_coef, u16 tap)
{
	...
	base = (volatile u32) (ipc->regs + filter_reg);
	coef = (u8*)filter_coef;
...
int ipc_init(u32 src_width, u32 src_height, enum ipc_2d ipc2d)
{
	...
	ipc_set_sharpness(ipc->enhance_var.sharpness, ipc->enhance_var.thhnoise);
