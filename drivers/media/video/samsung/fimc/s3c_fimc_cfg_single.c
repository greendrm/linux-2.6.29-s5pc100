...
void s3c_fimc_free_input_memory(struct s3c_fimc_in_frame *info)
{
	struct s3c_fimc_frame_addr *frame;
...
static int s3c_fimc_alloc_input_yuv_memory(struct s3c_fimc_in_frame *info)
{
	struct s3c_fimc_frame_addr *frame;
	int ret;
	u32 size = info->width * info->height, cbcr_size;
...
alloc_fail:
	s3c_fimc_free_input_memory(info);
	return ret;
}
...
int s3c_fimc_alloc_input_memory_volans(struct s3c_fimc_control *ctrl, 
					struct s3c_fimc_in_frame *info)
{
	int ret;
...
//	if (info->format == FORMAT_YCBCR420 || info->format == FORMAT_YCBCR422)
		ret = s3c_fimc_alloc_input_yuv_memory(info);
//	else
//		ret = s3c_fimc_alloc_rgb_memory_volans(info);
...
void s3c_fimc_start_dma(struct s3c_fimc_control *ctrl)
{
	...
	struct s3cfb_user_window win;

	dev_dbg(ctrl->dev, "[%d] start dma\n", ctrl->id);
	...
	if (ctrl->out_type == PATH_OUT_DMA) {
		...
		s3cfb_direct_ioctl(2, S3CFB_WIN_OFF, 0);
		s3cfb_direct_ioctl(1, FBIO_ALLOC, 0);
		s3cfb_direct_ioctl(1, FBIOGET_VSCREENINFO, \
					(unsigned long) &var);

		var.xres = ctrl->out_frame.width;
		var.yres = ctrl->out_frame.height;
		var.bits_per_pixel = 32;
		var.activate = FB_ACTIVATE_FORCE;

		s3cfb_direct_ioctl(1, FBIOPUT_VSCREENINFO, \
					(unsigned long) &var);

		win.x = ctrl->v4l2.crop_current.left;
		win.y = ctrl->v4l2.crop_current.top;

		s3cfb_direct_ioctl(1, S3CFB_WIN_POSITION, \
					(unsigned long) &win);
	}
	...
	if (ctrl->out_type == PATH_OUT_LCDFIFO)
		s3cfb_direct_ioctl(1, S3CFB_WIN_ON, 0);
}
...
{
	char *scdown = kmalloc(THUMB_SIZE, GFP_KERNEL);
	JPG_ENC_PROC_PARAM jpeg;

	jpeg.sampleMode = JPG_422;
	jpeg.encType = JPG_MAIN;
	jpeg.quality = frame->jpeg_quality;
	...
	/* copy for thumbnail */
	if (frame->jpeg_thumb) {
		memcpy(phys_to_virt(jpg_data_base_addr) + JPG_STREAM_BUF_SIZE + \
	...
	frame->jpeg_size = jpeg.fileSize;

	memcpy(frame->addr[index].virt_y, \
		phys_to_virt(jpg_data_base_addr), frame->jpeg_size);

	/* encode for thumbnail */
	if (frame->jpeg_thumb) {
		jpeg.sampleMode = JPG_422;
		jpeg.encType = JPG_THUMBNAIL;
		jpeg.quality = frame->jpeg_quality;
		jpeg.width = THUMB_WIDTH;
		jpeg.height = THUMB_HEIGHT;

		s3c_jpeg_direct_encode(&jpeg);
		frame->thumb_size = jpeg.fileSize;

		memcpy(frame->addr[index].virt_y + THUMB_OFFSET_JPEG, \
			phys_to_virt(jpg_data_base_addr) + JPG_STREAM_BUF_SIZE, \
			frame->thumb_size);
	} else {
		memcpy(frame->addr[index].virt_y + THUMB_OFFSET_YUV, \
