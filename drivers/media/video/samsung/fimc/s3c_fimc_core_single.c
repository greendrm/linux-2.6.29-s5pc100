...
static int s3c_fimc_mmap(struct file* filp, struct vm_area_struct *vma)
{
	...
	struct s3c_fimc_out_frame *out_frame = &ctrl->out_frame;
	struct s3c_fimc_in_frame *in_frame = &ctrl->in_frame;
	...
	if (ctrl->out_type == PATH_OUT_DMA) {
		total_size = out_frame->buf_size;
		pfn = __phys_to_pfn(out_frame->addr[vma->vm_pgoff].phys_y);
	} else if (ctrl->out_type == PATH_OUT_LCDFIFO) {
		total_size = in_frame->buf_size;
		pfn = __phys_to_pfn(in_frame->addr.phys_y);
	} else {
		err("mmap fail\n");
		return -EINVAL;
	}
