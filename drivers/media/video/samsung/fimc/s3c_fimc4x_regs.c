 ...
 * for now we use in_cam->id to identify camera A and B
...
void s3c_fimc_select_camera(struct s3c_fimc_control *ctrl)
{
	...
	if (ctrl->in_cam->type == CAM_TYPE_MIPI)
		cfg |= S3C_CIGCTRL_SELCAM_MIPI;
	else if (ctrl->in_cam->type == CAM_TYPE_ITU) {
		if (ctrl->in_cam->id == 0)
			cfg |= S3C_CIGCTRL_SELCAM_ITU_A;
		else
			cfg |= S3C_CIGCTRL_SELCAM_ITU_B;
	...
	if (ctrl->in_cam->type == CAM_TYPE_ITU) {
		cfg = readl(ctrl->regs + S3C_CISRCFMT);
		cfg |= ctrl->in_cam->mode;
		writel(cfg, ctrl->regs + S3C_CISRCFMT);
	} else if (ctrl->in_cam->type == CAM_TYPE_MIPI) {
...
void s3c_fimc_set_source_format(struct s3c_fimc_control *ctrl)
{
	...
	u32 cfg = 0;
#if 0
	/* FIXME: make it compatible with v4l2 pixelformat */
