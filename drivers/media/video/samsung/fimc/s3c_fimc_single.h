...
#ifndef _S3C_FIMC_SINGLE_H
#define _S3C_FIMC_SINGLE_H
...
#define FCLR_STATUS(x)			(x->flag &= ~S3C_FIMC_STA_MASK)
#define FCLR_USAGE(x)			(x->flag &= ~S3C_FIMC_USE_MASK)
#define FCLR_IRQ(x)			(x->flag &= ~S3C_FIMC_IRQ_MASK)

#define FSET_RUNNING(x)			FCLR_STATUS(x); (x->flag |= S3C_FIMC_FLAG_RUNNING)
#define FSET_STOP(x)			FCLR_STATUS(x); (x->flag |= S3C_FIMC_FLAG_STOP)
#define FSET_HANDLE_IRQ(x)		FCLR_STATUS(x); (x->flag |= S3C_FIMC_FLAG_HANDLE_IRQ)

#define FSET_PREVIEW(x)			FCLR_USAGE(x); (x->flag |= S3C_FIMC_FLAG_PREVIEW)
#define FSET_CAPTURE(x)			FCLR_USAGE(x); (x->flag |= S3C_FIMC_FLAG_CAPTURE)

#define FSET_IRQ_NORMAL(x)		FCLR_IRQ(x); (x->flag |= S3C_FIMC_FLAG_IRQ_NORMAL)
#define FSET_IRQ_X(x)			FCLR_IRQ(x); (x->flag |= S3C_FIMC_FLAG_IRQ_X)
#define FSET_IRQ_Y(x)			FCLR_IRQ(x); (x->flag |= S3C_FIMC_FLAG_IRQ_Y)
#define FSET_IRQ_LAST(x)		FCLR_IRQ(x); (x->flag |= S3C_FIMC_FLAG_IRQ_LAST)
...
extern int s3c_fimc_alloc_output_memory(struct s3c_fimc_control *ctrl, struct s3c_fimc_out_frame *info);
...
extern void s3c_fimc_free_input_memory(struct s3c_fimc_in_frame *info);
...
/* jpeg */
#define JPEG_BUFFER_SIZE		(1536 * 1200)
#define THUMB_WIDTH			320
#define THUMB_HEIGHT			240
#define THUMB_SIZE			(THUMB_WIDTH * THUMB_HEIGHT * 2)
#define THUMB_OFFSET_JPEG		0x1BD000
#define THUMB_OFFSET_YUV		(JPEG_BUFFER_SIZE - (THUMB_WIDTH * THUMB_HEIGHT * 2))

#define JPG_REG_BASE_ADDR		(0x78800000)
#define jpg_data_base_addr		(unsigned int)s3c_get_media_memory(S3C_MDEV_JPEG)
...
extern int s3c_jpeg_direct_encode(JPG_ENC_PROC_PARAM *EncParam);
extern int s3c_fimc_encode_jpeg(struct s3c_fimc_out_frame *frame, int index);

#endif /* _S3C_FIMC_SINGLE_H */
