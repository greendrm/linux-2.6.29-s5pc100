...
static irqreturn_t s3c_rp_rot_irq(int irq, void *dev_id)
{
	...
	if (ctrl->stream_status != RP_READY_OFF) {
		...
		if (ctrl->stream_status == RP_READY_ON) {
			...
			if (ret < 0) {
				rp_err(ctrl->log_level, "Failed : s3c_rp_attach_inside_queue.\n");
			}
		} else {
			if (ctrl->pp.buf_idx.next == -1) {
		...
		if (ret == 0) {
			...
			ctrl->rot.run_idx	= -1;
			rp_warn(ctrl->log_level, "[%s] : There is no buffer in incoming queue.\n", __FUNCTION__);
		}
	}
...
static int s3c_rp_open(struct inode *inode, struct file *filp)
{
	...
	printk( "Renderer pipeline Ver. 1.0.5\n");
...
static int s3c_rp_probe(struct platform_device *pdev)
{
	...
	if (!ctrl) {
		rp_err(ctrl->log_level, "cannot register renderer pipeline controller\n");
		goto err_rp;
	}
