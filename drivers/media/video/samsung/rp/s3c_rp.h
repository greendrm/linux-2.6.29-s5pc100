...
//#define info(args...)	do { printk(KERN_WARNING S3C_RP_NAME ": " args); } while (0)
...
enum s3c_rp_log_level {
	RP_LOG_DEBUG	= 0x1,		/* Queue status */
	RP_LOG_INFO	= 0x3,		/* V4L2 API */
	RP_LOG_WARN	= 0x7,		/* Warning conditions */
	RP_LOG_ERR	= 0xF,		/* Error conditions */
};
...
struct s3c_rp_control {
	...
	enum s3c_rp_log_level		log_level;
...
/* debug macro */
#define RP_DEBUG(num, fmt, ...)						\
	do {								\
		if (num <= RP_LOG_DEBUG)				\
			printk(KERN_DEBUG S3C_RP_NAME ": "		\
				fmt, ##__VA_ARGS__);			\
	} while(0)

#define RP_INFO(num, fmt, ...)						\
	do {								\
		if (num <= RP_LOG_INFO)					\
			printk(KERN_INFO S3C_RP_NAME ": "		\
				fmt, ##__VA_ARGS__);			\
	} while (0)

#define RP_WARN(num, fmt, ...)						\
	do {								\
		if (num <= RP_LOG_WARN)					\
			printk(KERN_WARNING S3C_RP_NAME ": "		\
				fmt, ##__VA_ARGS__);			\
	} while (0)


#define RP_ERROR(num, fmt, ...)						\
	do {								\
		if (num <= RP_LOG_ERR)					\
			printk(KERN_ERR S3C_RP_NAME ": "		\
				fmt, ##__VA_ARGS__);			\
	} while (0)


#define rp_dbg(num, fmt, ...)		RP_DEBUG(num, fmt, ##__VA_ARGS__)
#define rp_info(num, fmt, ...)		RP_INFO(num, fmt, ##__VA_ARGS__)
#define rp_warn(num, fmt, ...)		RP_WARN(num, fmt, ##__VA_ARGS__)
#define rp_err(num, fmt, ...)		RP_ERROR(num, fmt, ##__VA_ARGS__)
...
extern int s3c_rp_pp_query_scaler(struct s3c_rp_control *ctrl, const struct v4l2_rect *src_rect, const struct v4l2_rect *dst_rect, struct s3c_rp_scaler_cfg *scaler_cfg);
...
extern int s3c_rp_init_in_queue(struct s3c_rp_control *ctrl);
extern int s3c_rp_init_out_queue(struct s3c_rp_control *ctrl);
extern int s3c_rp_init_inside_queue(struct s3c_rp_control *ctrl);
...
extern int s3c_rp_sleep(void);
extern int s3c_rp_wakeup(void);
