...
s5p_tv_clk_err __s5p_tv_clk_init_mout_hpll(s5p_tv_clk_mout_hpll mout_hpll)
{
	...
	writel((readl(S5P_CLK_SRC1) &HDMI_SEL_MASK) | HDMI_SEL_HDMIPHY,
			S5P_CLK_SRC1);
	return S5P_TV_CLK_ERR_NO_ERROR;
}
...
s5p_tv_clk_err __s5p_tv_clk_init_video_mixer(s5p_tv_clk_vmiexr_srcclk src_clk)
{
	...
	/* temp. VPLLSRC_SEL FINPLL */
	//writel((1<<28) | readl(S5P_CLK_SRC1), S5P_CLK_SRC1);
	...
	case TVOUT_CLK_VMIXER_SRCCLK_VCLK_54:
		writel(((readl(S5P_CLK_SRC1) &VMIXER_SEL_MASK) | VMIXER_SEL_VCLK_54), 
			S5P_CLK_SRC1);
		break;

	case TVOUT_CLK_VMIXER_SRCCLK_MOUT_HPLL:
		writel(((readl(S5P_CLK_SRC1) &VMIXER_SEL_MASK) | VMIXER_SEL_MOUT_HPLL),
			S5P_CLK_SRC1);
		break;
...
void __s5p_tv_clk_set_vp_clk_onoff(bool clk_on)
{
	...
	if (clk_on) {
		bit_add_l(CLK_HCLK_VP_PASS, S5P_CLKGATE_MAIN1);
	} else {
		bit_del_l(CLK_HCLK_VP_PASS, S5P_CLKGATE_MAIN1);	
	}
...
void __s5p_tv_clk_set_vmixer_hclk_onoff(bool clk_on)
{
	...
	if (clk_on) {
		bit_add_l(CLK_HCLK_VMIXER_PASS, S5P_CLKGATE_MAIN1);
	} else {
		bit_del_l(CLK_HCLK_VMIXER_PASS, S5P_CLKGATE_MAIN1);
	}
