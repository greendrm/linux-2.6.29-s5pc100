...
void __s5p_hdmi_set_hpd_onoff(bool on_off)
{
	...
	if (on_off) {
		writel(SW_HPD_PLUGGED, tvout_base + S5P_HPD);
	} else {
		writel(SW_HPD_UNPLUGGED, tvout_base + S5P_HPD);
	}
...
void __s5p_hdmi_audio_set_config(s5p_tv_audio_codec_type audio_codec)
{
	...
	writel(CONFIG_FILTER_2_SAMPLE | data_type
	       ...
	       , tvout_base + S5P_SPDIFIN_CONFIG_1);
	writel(0, tvout_base + S5P_SPDIFIN_CONFIG_2);
	HDMIPRINTK("()\n\r");
}
...
void __s5p_hdmi_audio_set_acr(u32 sample_rate)
{
	...
	writel(value_n & 0xff, tvout_base + S5P_ACR_N0);
	writel((value_n >> 8) & 0xff, tvout_base + S5P_ACR_N1);
	writel((value_n >> 16) & 0xff, tvout_base + S5P_ACR_N2);

	writel(cts & 0xff, tvout_base + S5P_ACR_MCTS0);
	writel((cts >> 8) & 0xff, tvout_base + S5P_ACR_MCTS1);
	writel((cts >> 16) & 0xff, tvout_base + S5P_ACR_MCTS2);

	writel(cts & 0xff, tvout_base + S5P_ACR_CTS0);
	writel((cts >> 8) & 0xff, tvout_base + S5P_ACR_CTS1);
	writel((cts >> 16) & 0xff, tvout_base + S5P_ACR_CTS2);

	writel(4, tvout_base + S5P_ACR_CON); //TO-DO : what it is?
...
void __s5p_hdmi_audio_set_asp(void)
{
	...
	writel(1 << 3 | 0, tvout_base + S5P_ASP_CHCFG2);
	writel(1 << 3 | 0, tvout_base + S5P_ASP_CHCFG3);
	HDMIPRINTK("()\n\r");
}
...
{
	HDMIPRINTK("()\n\r");
	writel(0x1, tvout_base + S5P_SPDIFIN_CLK_CTRL);
	writel(0x3, tvout_base + S5P_SPDIFIN_OP_CTRL); // HDMI operation mode
	HDMIPRINTK("()\n\r");
}
					...
					u32 bits, u32 frame_size_code)
{
	...
	writel(((rpt_cnt&0xf) << 4) | wl, tvout_base + S5P_SPDIFIN_USER_VALUE_1); 
	// if PCM this value is 0
	writel((rpt_cnt >> 4)&0xff, tvout_base + S5P_SPDIFIN_USER_VALUE_2);
	// if PCM this value is 0
	writel(frame_size_code&0xff, tvout_base + S5P_SPDIFIN_USER_VALUE_3);	
	// if PCM this value is 0
	writel((frame_size_code >> 8)&0xff, tvout_base + S5P_SPDIFIN_USER_VALUE_4);
	HDMIPRINTK("()\n\r");
}
...
{
	writel(irq_en, tvout_base + S5P_SPDIFIN_IRQ_MASK);
}
			...
			u32 bits)
{
	...
	// AUI Packet set.
	writel(check_sum , tvout_base + S5P_AUI_CHECK_SUM);
	writel(bytes1 , tvout_base + S5P_AUI_BYTE1);
	writel(bytes2 , tvout_base + S5P_AUI_BYTE2);
	writel(bytes3 , tvout_base + S5P_AUI_BYTE3); // Pcm or stream
	writel(0x00 , tvout_base + S5P_AUI_BYTE4); // 2ch pcm or Stream
	writel(0x00 , tvout_base + S5P_AUI_BYTE5); // 2ch pcm or Stream


	writel(2 , tvout_base + S5P_ACP_CON);
	writel(1 , tvout_base + S5P_ACP_TYPE);

	writel(0x10 , tvout_base + S5P_GCP_BYTE1);
	// packet will be transmitted within 384 cycles after active sync.
	writel(0x2 , tvout_base + S5P_GCP_CON);  
				     ...
				     u8 cr_r)
{
	...
	if (en) {
		writel(SET_BLUESCREEN_0(cb_b), tvout_base + S5P_BLUE_SCREEN_0);
		writel(SET_BLUESCREEN_1(y_g), tvout_base + S5P_BLUE_SCREEN_1);
		writel(SET_BLUESCREEN_2(cr_r), tvout_base + S5P_BLUE_SCREEN_2);
		writel(readl(tvout_base + S5P_HDMI_CON_0) | BLUE_SCR_EN, 
			tvout_base + S5P_HDMI_CON_0);

		HDMIPRINTK("HDMI_BLUE_SCREEN0 = 0x%08x \n\r", 
			readl(tvout_base + S5P_BLUE_SCREEN_0));
		HDMIPRINTK("HDMI_BLUE_SCREEN1 = 0x%08x \n\r", 
			readl(tvout_base + S5P_BLUE_SCREEN_1));
		HDMIPRINTK("HDMI_BLUE_SCREEN2 = 0x%08x \n\r", 
			readl(tvout_base + S5P_BLUE_SCREEN_2));
	} else {
		writel(readl(tvout_base + S5P_HDMI_CON_0)&~BLUE_SCR_EN, 
			tvout_base + S5P_HDMI_CON_0);
	}

	HDMIPRINTK("HDMI_CON0 = 0x%08x \n\r", readl(tvout_base + S5P_HDMI_CON_0));
}
						    ...
						    u8 *spd_data)
{
	...
	case HDMI_DO_NOT_TANS:
		writel(SPD_TX_CON_NO_TRANS, tvout_base + S5P_SPD_CON);
		break;

	case HDMI_TRANS_ONCE:
		writel(SPD_TX_CON_TRANS_ONCE, tvout_base + S5P_SPD_CON);
		break;

	case HDMI_TRANS_EVERY_SYNC:
		writel(SPD_TX_CON_TRANS_EVERY_VSYNC, tvout_base + S5P_SPD_CON);
		break;
	...
	writel(SET_SPD_HEADER(*(spd_header)), tvout_base + S5P_SPD_HEADER0);

	writel(SET_SPD_HEADER(*(spd_header + 1)) , tvout_base + S5P_SPD_HEADER1);
	writel(SET_SPD_HEADER(*(spd_header + 2)) , tvout_base + S5P_SPD_HEADER2);

	writel(SET_SPD_DATA(*(spd_data)), tvout_base + S5P_SPD_DATA0);
	writel(SET_SPD_DATA(*(spd_data + 1)) , tvout_base + S5P_SPD_DATA1);
	writel(SET_SPD_DATA(*(spd_data + 2)) , tvout_base + S5P_SPD_DATA2);
	writel(SET_SPD_DATA(*(spd_data + 3)) , tvout_base + S5P_SPD_DATA3);
	writel(SET_SPD_DATA(*(spd_data + 4)) , tvout_base + S5P_SPD_DATA4);
	writel(SET_SPD_DATA(*(spd_data + 5)) , tvout_base + S5P_SPD_DATA5);
	writel(SET_SPD_DATA(*(spd_data + 6)) , tvout_base + S5P_SPD_DATA6);
	writel(SET_SPD_DATA(*(spd_data + 7)) , tvout_base + S5P_SPD_DATA7);
	writel(SET_SPD_DATA(*(spd_data + 8)) , tvout_base + S5P_SPD_DATA8);
	writel(SET_SPD_DATA(*(spd_data + 9)) , tvout_base + S5P_SPD_DATA9);
	writel(SET_SPD_DATA(*(spd_data + 10)) , tvout_base + S5P_SPD_DATA10);
	writel(SET_SPD_DATA(*(spd_data + 11)) , tvout_base + S5P_SPD_DATA11);
	writel(SET_SPD_DATA(*(spd_data + 12)) , tvout_base + S5P_SPD_DATA12);
	writel(SET_SPD_DATA(*(spd_data + 13)) , tvout_base + S5P_SPD_DATA13);
	writel(SET_SPD_DATA(*(spd_data + 14)) , tvout_base + S5P_SPD_DATA14);
	writel(SET_SPD_DATA(*(spd_data + 15)) , tvout_base + S5P_SPD_DATA15);
	writel(SET_SPD_DATA(*(spd_data + 16)) , tvout_base + S5P_SPD_DATA16);
	writel(SET_SPD_DATA(*(spd_data + 17)) , tvout_base + S5P_SPD_DATA17);
	writel(SET_SPD_DATA(*(spd_data + 18)) , tvout_base + S5P_SPD_DATA18);
	writel(SET_SPD_DATA(*(spd_data + 19)) , tvout_base + S5P_SPD_DATA19);
	writel(SET_SPD_DATA(*(spd_data + 20)) , tvout_base + S5P_SPD_DATA20);
	writel(SET_SPD_DATA(*(spd_data + 21)) , tvout_base + S5P_SPD_DATA21);
	writel(SET_SPD_DATA(*(spd_data + 22)) , tvout_base + S5P_SPD_DATA22);
	writel(SET_SPD_DATA(*(spd_data + 23)) , tvout_base + S5P_SPD_DATA23);
	writel(SET_SPD_DATA(*(spd_data + 24)) , tvout_base + S5P_SPD_DATA24);
	writel(SET_SPD_DATA(*(spd_data + 25)) , tvout_base + S5P_SPD_DATA25);
	writel(SET_SPD_DATA(*(spd_data + 26)) , tvout_base + S5P_SPD_DATA26);
	writel(SET_SPD_DATA(*(spd_data + 27)) , tvout_base + S5P_SPD_DATA27);

	HDMIPRINTK("SPD_CON = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_CON));
	HDMIPRINTK("SPD_HEADER0 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_HEADER0));
	HDMIPRINTK("SPD_HEADER1 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_HEADER1));
	HDMIPRINTK("SPD_HEADER2 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_HEADER2));
	HDMIPRINTK("SPD_DATA0  = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA0));
	HDMIPRINTK("SPD_DATA1  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA1));
	HDMIPRINTK("SPD_DATA2  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA2));
	HDMIPRINTK("SPD_DATA3  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA3));
	HDMIPRINTK("SPD_DATA4  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA4));
	HDMIPRINTK("SPD_DATA5  = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA5));
	HDMIPRINTK("SPD_DATA6  = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA6));
	HDMIPRINTK("SPD_DATA7  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA7));
	HDMIPRINTK("SPD_DATA8  = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA8));
	HDMIPRINTK("SPD_DATA9  = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA9));
	HDMIPRINTK("SPD_DATA10 = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA10));
	HDMIPRINTK("SPD_DATA11 = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA11));
	HDMIPRINTK("SPD_DATA12 = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA12));
	HDMIPRINTK("SPD_DATA13 = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA13));
	HDMIPRINTK("SPD_DATA14 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA14));
	HDMIPRINTK("SPD_DATA15 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA15));
	HDMIPRINTK("SPD_DATA16 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA16));
	HDMIPRINTK("SPD_DATA17 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA17));
	HDMIPRINTK("SPD_DATA18 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA18));
	HDMIPRINTK("SPD_DATA19 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA19));
	HDMIPRINTK("SPD_DATA20 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA20));
	HDMIPRINTK("SPD_DATA21 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA21));
	HDMIPRINTK("SPD_DATA22 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA22));
	HDMIPRINTK("SPD_DATA23 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA23));
	HDMIPRINTK("SPD_DATA24 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA24));
	HDMIPRINTK("SPD_DATA25 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA25));
	HDMIPRINTK("SPD_DATA26 = 0x%08x \n\r",
		readl(tvout_base + S5P_SPD_DATA26));
	HDMIPRINTK("SPD_DATA27 = 0x%08x \n\r", 
		readl(tvout_base + S5P_SPD_DATA27));
						...
						s5p_tv_o_mode out_mode)
{
	...
	case TVOUT_720P_50:
		writel(INT_PRO_MODE_PROGRESSIVE, tvout_base + S5P_INT_PRO_MODE);
		break;
	...
	case TVOUT_OUTPUT_HDMI:
		writel(PX_LMT_CTRL_BYPASS, tvout_base + S5P_HDMI_CON_1);
		writel(VID_PREAMBLE_EN | GUARD_BAND_EN, tvout_base + S5P_HDMI_CON_2);
		writel(HDMI_MODE_EN | DVI_MODE_DIS, tvout_base + S5P_MODE_SEL);
		break;
	...
	writel(SET_V_LINE_H(g_hdmi_video_parm_tbl[V_LINE*S5P_TV_HDMI_DISP_MODE_NUM + hdmi_disp_num]) |
	       ...
	       tvout_base + S5P_H_V_LINE_1);
	writel(SET_H_LINE_H(g_hdmi_video_parm_tbl[H_LINE*S5P_TV_HDMI_DISP_MODE_NUM + hdmi_disp_num]),
	       tvout_base + S5P_H_V_LINE_2);

	writel(g_hdmi_video_parm_tbl[VSYNC_POL*S5P_TV_HDMI_DISP_MODE_NUM + hdmi_disp_num] , tvout_base + S5P_SYNC_MODE);

	writel(0xfd, tvout_base + S5P_SEND_PER_START0);
	writel(0x01, tvout_base + S5P_SEND_PER_START1);
	writel(0x0d, tvout_base + S5P_SEND_PER_END0);
	writel(0x3a, tvout_base + S5P_SEND_PER_END1);
	writel(0x08, tvout_base + S5P_SEND_PER_END2);

	writel(SET_V_BOT_ST_L(g_hdmi_video_parm_tbl[V_BOT_ST*S5P_TV_HDMI_DISP_MODE_NUM + hdmi_disp_num]) ,
	       tvout_base + S5P_V_BLANK_F_0);
				     ...
				     u8 c_max)
{
	...
	writel(y_max, tvout_base + S5P_HDMI_YMAX);
	writel(y_min, tvout_base + S5P_HDMI_YMIN);
	writel(c_max, tvout_base + S5P_HDMI_CMAX);
	writel(c_min, tvout_base + S5P_HDMI_CMIN);

	HDMIPRINTK("HDMI_YMAX = 0x%08x \n\r", readl(tvout_base + S5P_HDMI_YMAX));
	HDMIPRINTK("HDMI_YMIN = 0x%08x \n\r", readl(tvout_base + S5P_HDMI_YMIN));
	HDMIPRINTK("HDMI_CMAX = 0x%08x \n\r", readl(tvout_base + S5P_HDMI_CMAX));
	HDMIPRINTK("HDMI_CMIN = 0x%08x \n\r", readl(tvout_base + S5P_HDMI_CMIN));
}
						  ...
						  u8 *avi_data)
{
	...
	case HDMI_DO_NOT_TANS:
		writel(AVI_TX_CON_NO_TRANS, tvout_base + S5P_AVI_CON);
		break;

	case HDMI_TRANS_ONCE:
		writel(AVI_TX_CON_TRANS_ONCE, tvout_base + S5P_AVI_CON);
		break;

	case HDMI_TRANS_EVERY_SYNC:
		writel(AVI_TX_CON_TRANS_EVERY_VSYNC, tvout_base + S5P_AVI_CON);
		break;
	...
	writel(SET_AVI_CHECK_SUM(check_sum), tvout_base + S5P_AVI_CHECK_SUM);

	writel(SET_AVI_BYTE(*(avi_data)), tvout_base + S5P_AVI_BYTE1);
	writel(SET_AVI_BYTE(*(avi_data + 1)), tvout_base + S5P_AVI_BYTE2);
	writel(SET_AVI_BYTE(*(avi_data + 2)), tvout_base + S5P_AVI_BYTE3);
	writel(SET_AVI_BYTE(*(avi_data + 3)), tvout_base + S5P_AVI_BYTE4);
	writel(SET_AVI_BYTE(*(avi_data + 4)), tvout_base + S5P_AVI_BYTE5);
	writel(SET_AVI_BYTE(*(avi_data + 5)), tvout_base + S5P_AVI_BYTE6);
	writel(SET_AVI_BYTE(*(avi_data + 6)), tvout_base + S5P_AVI_BYTE7);
	writel(SET_AVI_BYTE(*(avi_data + 7)), tvout_base + S5P_AVI_BYTE8);
	writel(SET_AVI_BYTE(*(avi_data + 8)), tvout_base + S5P_AVI_BYTE9);
	writel(SET_AVI_BYTE(*(avi_data + 9)), tvout_base + S5P_AVI_BYTE10);
	writel(SET_AVI_BYTE(*(avi_data + 10)), tvout_base + S5P_AVI_BYTE11);
	writel(SET_AVI_BYTE(*(avi_data + 11)), tvout_base + S5P_AVI_BYTE12);
	writel(SET_AVI_BYTE(*(avi_data + 12)), tvout_base + S5P_AVI_BYTE13);

	HDMIPRINTK("AVI_CON = 0x%08x \n\r", readl(tvout_base + S5P_AVI_CON));
	HDMIPRINTK("AVI_CHECK_SUM = 0x%08x \n\r", readl(tvout_base + S5P_AVI_CHECK_SUM));
	HDMIPRINTK("AVI_BYTE1  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE1));
	HDMIPRINTK("AVI_BYTE2  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE2));
	HDMIPRINTK("AVI_BYTE3  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE3));
	HDMIPRINTK("AVI_BYTE4  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE4));
	HDMIPRINTK("AVI_BYTE5  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE5));
	HDMIPRINTK("AVI_BYTE6  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE6));
	HDMIPRINTK("AVI_BYTE7  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE7));
	HDMIPRINTK("AVI_BYTE8  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE8));
	HDMIPRINTK("AVI_BYTE9  = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE9));
	HDMIPRINTK("AVI_BYTE10 = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE10));
	HDMIPRINTK("AVI_BYTE11 = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE11));
	HDMIPRINTK("AVI_BYTE12 = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE12));
	HDMIPRINTK("AVI_BYTE13 = 0x%08x \n\r", readl(tvout_base + S5P_AVI_BYTE13));
						...
						u8 *mpg_data)
{
	...
	writel(SET_MPG_CHECK_SUM(check_sum),
	       ...
	       tvout_base + S5P_MPG_CHECK_SUM);

	writel(SET_MPG_BYTE(*(mpg_data)),
	       tvout_base + S5P_MPEG_BYTE1);
	writel(SET_MPG_BYTE(*(mpg_data + 1)),
	       tvout_base + S5P_MPEG_BYTE2);
	writel(SET_MPG_BYTE(*(mpg_data + 2)),
	       tvout_base + S5P_MPEG_BYTE3);
	writel(SET_MPG_BYTE(*(mpg_data + 3)),
	       tvout_base + S5P_MPEG_BYTE4);
	writel(SET_MPG_BYTE(*(mpg_data + 4)),
	       tvout_base + S5P_MPEG_BYTE5);

	HDMIPRINTK("MPG_CON = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPG_CON));
	HDMIPRINTK("MPG_CHECK_SUM = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPG_CHECK_SUM));
	HDMIPRINTK("MPEG_BYTE1 = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPEG_BYTE1));
	HDMIPRINTK("MPEG_BYTE2 = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPEG_BYTE2));
	HDMIPRINTK("MPEG_BYTE3 = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPEG_BYTE3));
	HDMIPRINTK("MPEG_BYTE4 = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPEG_BYTE4));
	HDMIPRINTK("MPEG_BYTE5 = 0x%08x \n\r",
		   readl(tvout_base + S5P_MPEG_BYTE5));
				...
				bool tg_en)
{
	...
	writel(temp_reg, tvout_base + S5P_TG_CMD);

	HDMIPRINTK("TG_CMD = 0x%08x \n\r",
		   readl(tvout_base + S5P_TG_CMD));
}
				...
				struct i2c_client *ddc_port)
{
	...
	writel(readl(tvout_base + S5P_HDMI_CON_0) | temp_reg,
	       tvout_base + S5P_HDMI_CON_0);
	...
	HDMIPRINTK("HDCP_CTRL : 0x%08x, HPD : 0x%08x, HDMI_CON_0 : 0x%08x\n\r",
		   ...
		   readl(tvout_base + S5P_HPD),
		   readl(tvout_base + S5P_HDMI_CON_0));
...
void __s5p_hdmi_stop(void)
{
	...
	writel(readl(tvout_base + S5P_HDMI_CON_0) &
