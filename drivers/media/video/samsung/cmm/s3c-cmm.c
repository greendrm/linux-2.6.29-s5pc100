...
int s3c_cmm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if ((offset == 0) && (S3C_CMM_CODEC_CACHED_MEM_SIZE != 0)) {
...
static int __init s3c_cmm_init(void)
{
	...
	s3c_cmm_buffer_start = s3c_get_media_memory_node(S3C_MDEV_CMM, 1);
...
static void release_alloc_mem(s3c_cmm_alloc_mem_t *node, s3c_cmm_codec_mem_ctx_t *CodecMem)
{
...
static s3c_cmm_alloc_mem_t *get_codec_virt_addr(int inst_no, s3c_cmm_codec_mem_alloc_arg_t *in_param)
{
	...
	p_startAddr = get_mem_area((int)in_param->buffSize, inst_no, in_param->cacheFlag);
...
static void print_list()
{
	...
	for (node1 = s3c_cmm_allocmem_head; node1 != s3c_cmm_allocmem_tail; node1 = node1->next) {
		if (node1->cacheFlag)
	...
	count = 0;
	for(node2 = s3c_cmm_freemem_head; node2 != s3c_cmm_freemem_tail; node2 = node2->next) {
